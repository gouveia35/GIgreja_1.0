...
class cusuarios extends cTable {
	...
	function __construct() {
		...
		// u
		$this->u = new cField('usuarios', 'usuarios', 'x_u', 'u', '`u`', '`u`', 3, -1, FALSE, '`u`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->u->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u'] = &$this->u;

		// Nome
		$this->Nome = new cField('usuarios', 'usuarios', 'x_Nome', 'Nome', '`Nome`', '`Nome`', 200, -1, FALSE, '`Nome`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Nome'] = &$this->Nome;

		// login
		$this->_login = new cField('usuarios', 'usuarios', 'x__login', 'login', '`login`', '`login`', 200, -1, FALSE, '`login`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['login'] = &$this->_login;

		// senha
		$this->senha = new cField('usuarios', 'usuarios', 'x_senha', 'senha', '`senha`', '`senha`', 200, -1, FALSE, '`senha`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// Level
		$this->Level = new cField('usuarios', 'usuarios', 'x_Level', 'Level', '`Level`', '`Level`', 3, -1, FALSE, '`Level`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('u', $rs))
				ew_AddFilter($where, ew_QuotedName('u') . '=' . ew_QuotedValue($rs['u'], $this->u->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->u->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@u@", ew_AdjustSql($this->u->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->u->CurrentValue)) {
			$sUrl .= "u=" . urlencode($this->u->CurrentValue);
		} else {
   ...
   // Common render codes
	...
	function UserIDFilter($userid) {
		$sUserIDFilter = '`u` = ' . ew_QuotedValue($userid, EW_DATATYPE_NUMBER);
