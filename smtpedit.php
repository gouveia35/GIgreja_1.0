...
$smtp_edit = NULL; // Initialize page object first

class csmtp_edit extends csmtp {
	...
	function Page_Main() {
		...
		if ($this->Id->CurrentValue == "")
			$this->Page_Terminate("smtplist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// SMTP
			$this->SMTP->SetDbValueDef($rsnew, $this->SMTP->CurrentValue, NULL, $this->SMTP->ReadOnly);

			// SMTP_Porta
			$this->SMTP_Porta->SetDbValueDef($rsnew, $this->SMTP_Porta->CurrentValue, NULL, $this->SMTP_Porta->ReadOnly);

			// SMTP_Usuario
			$this->SMTP_Usuario->SetDbValueDef($rsnew, $this->SMTP_Usuario->CurrentValue, NULL, $this->SMTP_Usuario->ReadOnly);

			// SMTP_Senha
			$this->SMTP_Senha->SetDbValueDef($rsnew, $this->SMTP_Senha->CurrentValue, NULL, $this->SMTP_Senha->ReadOnly);

			// Email_de_Envio
			$this->Email_de_Envio->SetDbValueDef($rsnew, $this->Email_de_Envio->CurrentValue, NULL, $this->Email_de_Envio->ReadOnly);

			// Email_de_Recebimento
			$this->Email_de_Recebimento->SetDbValueDef($rsnew, $this->Email_de_Recebimento->CurrentValue, NULL, $this->Email_de_Recebimento->ReadOnly);

			// Seguranca
			$this->Seguranca->SetDbValueDef($rsnew, $this->Seguranca->CurrentValue, NULL, $this->Seguranca->ReadOnly);
...
// Create page object
if (!isset($smtp_edit)) $smtp_edit = new csmtp_edit();
...
$smtp_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var smtp_edit = new ew_Page("smtp_edit");
smtp_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = smtp_edit.PageID; // For backward compatibility

// Form object
var fsmtpedit = new ew_Form("fsmtpedit");

// Validate form
fsmtpedit.Validate = function() {
...
fsmtpedit.ValidateRequired = true;
<?php } else { ?>
fsmtpedit.ValidateRequired = false; 
<?php } ?>
...
<?php $smtp_edit->ShowPageHeader(); ?>
<?php
$smtp_edit->ShowMessage();
?>
<form name="fsmtpedit" id="fsmtpedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($smtp_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $smtp_edit->Token ?>">
<?php } ?>
...
<input type="password" data-field="x_SMTP_Senha" name="x_SMTP_Senha" id="x_SMTP_Senha" value="<?php echo $smtp->SMTP_Senha->EditValue ?>" size="20" maxlength="50"<?php echo $smtp->SMTP_Senha->EditAttributes() ?>>
</span>
...
<span id="el_smtp_Id">
<input type="hidden" data-field="x_Id" name="x_Id" id="x_Id" value="<?php echo ew_HtmlEncode($smtp->Id->CurrentValue) ?>">
</span>
